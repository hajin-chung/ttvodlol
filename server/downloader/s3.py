...
# S3_BUCKET = "ttvodlol"
# S3_KEY = "AKIAXFLBOG3DKWWSTGLA"
# S3_SECRET = "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"
...
def upload_s3(file_name, content):
    method = 'PUT'
    ...
    pre_auth_headers = {}
    query = {}
    headers = aws_sig_v4_headers(S3_KEY, S3_SECRET, pre_auth_headers,
                                 's3', 'ap-northeast-2', host, method, path, query, data)
    print("[INFO:2] writing to s3 ", f"https://{host}{path}")
    res = requests.put(f'https://{host}{path}', headers=headers, data=data)
    print("<- ", res)